...
import { getFatture, svuotaFattura, trovaIdFattura } from "../redux/action";
...
function Fatture(props) {
  ...
  const creaPDF = () => {
    ...
    doc.setFontSize(7);
    doc.text(150, 15, `${ordini[props.id].azienda.nome}`);
    doc.setFont("Helvetica", "normal");
    doc.text(150, 20, `Via Italia, 85/P - 25525 - ${ordini[props.id].azienda.città}`);
    doc.text(
      150,
      25,
      `P.iva IT${ordini[props.id].azienda.partitaIva} - C.F. ${ordini[props.id].azienda.codiceFiscale}`
    );
    doc.text(
      150,
      30,
      `FATTURA nr. ${ordini[props.id].id}/${ordini[props.id].dataOrdine.slice(0, 4)} del ${ordini[props.id].dataOrdine}`
    );
    doc.line(20, 40, 190, 40, "S");

    /* PARTE CENTRALE SOPRA */
    doc.setFont("Helvetica", "bold");
    doc.text(20, 45, `P.IVA IT${ordini[props.id].azienda.partitaIva}`);
    doc.text(20, 50, `C.F. ${ordini[props.id].azienda.codiceFiscale}`);
    doc.text(110, 45, `DESTINATARIO`);
    doc.setFont("Helvetica", "normal");
    doc.text(110, 50, `${ordini[props.id].user.firstname} ${ordini[props.id].user.lastname}`);
    doc.text(110, 55, `${ordini[props.id].user.indirizzo}`);
    ...
    doc.text(120, 240, `${ordini[props.id].dataConsegna}`);
    doc.setFont("Helvetica", "normal");
    ...
    doc.setFont("Helvetica", "normal");
    doc.text(150, 255, `Importo totale € ${fatture[props.id].importoTotale.toFixed(2)}`);
    doc.text(150, 260, `Prezzo consegna € ${ordini[props.id].prezzoConsegna}`);
    doc.setFontSize(20);
    doc.setFont("Helvetica", "bold");
    doc.text(150, 270, `€ ${fatture[props.id].importoTotale.toFixed(2)}`);
    doc.setFontSize(6);
    doc.text(170, 290, `${ordini[props.id].azienda.nome}`);
    doc.text(
      20,
      290,
      `FATTURA nr. ${ordini[props.id].id}/${ordini[props.id].dataOrdine.slice(0, 4)} del ${ordini[props.id].dataOrdine}`
